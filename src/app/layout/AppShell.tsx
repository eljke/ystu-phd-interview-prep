...
export function AppShell() {
  ...
  const visibleLinks =
    role === 'admin'
      ? [...links, { to: '/admin/access', label: 'Доступ', icon: ShieldCheck }]
      : links;
  return (
    <div className="app-shell">
      ...
      <header className="mobile-header">
        <button className="icon-button" aria-label="Открыть меню" onClick={() => setOpen(true)}>
          <Menu />
        </button>
        <strong>ЯГТУ · 1.2.2</strong>
      </header>
      <aside className={`sidebar ${open ? 'sidebar--open' : ''}`}>
        ...
        <div className="sidebar__footer">
          ...
          {cloudEnabled && (
            <button className="theme-button" onClick={() => void signOut()}>
              <LogOut size={18} />
              <span>Выйти</span>
            </button>
          )}
          {noSaveMode && <p className="no-save-badge">Режим без сохранения</p>}
        </div>
      </aside>
      {open && (
        <button className="backdrop" aria-label="Закрыть меню" onClick={() => setOpen(false)} />
      )}
