...
export function ProfileProvider({children}:{children:ReactNode}) {
  ...
  const setActiveProfileId=async(id:string)=>{ setActive(id); const settings=await repository.getSettings(); await repository.saveSettings({...settings,activeProfileId:id,updatedAt:new Date().toISOString()}); };
  const value=useMemo(()=>({profiles,activeProfile:profiles.find((p)=>p.id===activeProfileId),activeProfileId,setActiveProfileId,refreshProfiles}),[profiles,activeProfileId,refreshProfiles]);
