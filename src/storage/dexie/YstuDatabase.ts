...
export class YstuDatabase extends Dexie {
  ...
  async migrateLegacyYagtuDatabase() {
    ...
    try {
      await legacy.open();
      const [profiles, topicProgress, quizAttempts, oralAttempts, partnerAssessments, studySessions, settings] = await Promise.all([
      ...
      await this.transaction('rw', [this.profiles, this.topicProgress, this.quizAttempts, this.oralAttempts, this.partnerAssessments, this.studySessions, this.settings, this.migrations], async () => {
        await Promise.all([
